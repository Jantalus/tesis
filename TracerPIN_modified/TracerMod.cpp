...
const uint64_t buffer_size = 500 * 1000 * 1024; // 10 MB
std::vector<char> traceBuffer(buffer_size); 
...
KNOB<std::string> KnobVariableName(KNOB_MODE_WRITEONCE, "pintool",
                                   "vname", "0", "Variable name, will be used to lookup in the dwarf debug data. \n Default (0) means no use of this filter. Use with <PONER ACA SIZE IN BYTES Y FUNC OPT>");
...
KNOB<std::string> KnobFunctionName(KNOB_MODE_WRITEONCE, "pintool",
                                   "fname", "0", "Function  name, will be used to lookup in the dwarf debug data. \n Default (0) means no use of this filter. Use with <PONER ACA SIZE IN BYTES Y FUNC OPT>");
...
int main(int argc, char *argv[])
{
    ...
    // g_timer = new FunctionTimer();

    // Start timing for main function

    /*
    if (g_timer)
        g_timer->startTimer("main");
    */
    ...
    TraceFile.rdbuf()->pubsetbuf(traceBuffer.data(), buffer_size);
    if (TraceFile.fail())
    {
